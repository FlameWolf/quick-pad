	...
	const sections: LegalSection[] = [
		...
		},
		{
			...
			blocks: [
				...
				{
					type: "list",
					items: ["Google — contacted only for the optional Drive sync and for sign-in. On your device, the app talks to Google's Drive v3 file endpoints. On QuickPad's server side, the backend contacts Google's OAuth endpoints for authorization (https://accounts.google.com/o/oauth2/v2/auth), token exchange and refresh (https://oauth2.googleapis.com/token), and token revocation on sign-out (https://oauth2.googleapis.com/revoke). Note data is sent to Google only when you sign in and a sync runs.", "Vercel — the hosting platform that serves the app and runs the small serverless functions used for the Google sign-in, token, and sign-out flow.", "Google Identity Services — the page loads Google's sign-in client script from https://accounts.google.com/gsi/client (with a DNS-prefetch hint to the same Google origin), so your browser contacts Google's servers when loading the page. This is the only third-party script the page loads."]
				},
...
<template>
	<LegalPage title="Privacy Policy" effective-date="12 June 2026" :intro="intro" :sections="sections"/>
</template>